...
public class ResourceReleaseRule implements Rule, Obsoletable {
	...
	public ResourceReleaseRule(String resourceId, List<Predicate> predicates, Conjunction conj) {
	...
	private List<Predicate> reconstitutePredicates(Element element) {
		List<Predicate> rv = new ArrayList<Predicate>();
		try {
			Predicate aPredicate = null;
			...
			for (int i = 0; i < length; i++) {
				...
				if (predicate.getTagName().equals("predicate")) {
					...
					aPredicate = (Predicate) Class.forName(className).newInstance();
